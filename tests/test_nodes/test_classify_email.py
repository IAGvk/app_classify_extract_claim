...
async def test_webform_detected_by_heuristics(webform_state, mock_settings):
    with patch(
        "app_classify_extract_claim.graph.nodes.classify_email.get_settings",
...
async def test_freetext_email_classified_by_llm(motor_state, mock_settings):
    ...
    mock_client.ainvoke_structured = AsyncMock(
        return_value=EmailTypeResponse(email_type="freetext")
    )
    ...
    with (
        ...
        patch(
            "app_classify_extract_claim.graph.nodes.classify_email.llm_module.LLMClient.from_settings",
